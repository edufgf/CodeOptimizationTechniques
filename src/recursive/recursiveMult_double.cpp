...
void matrixMultiplication(int A_ini_i, int A_ini_j, int A_dim1, int A_dim2, int B_ini_i, int B_ini_j, int B_dim2,
    ...
    int A_morton_next_block = (A_morton_i+1)*g_mortonBlockSide;
    int B_morton_next_block = (B_morton_j+1)*g_mortonBlockSide;
    int B_aux_next_block = B_morton_next_block;
    int change_A = 0;
    int change_B = 0;
    int change_C = 0;
    ...
    for (A_index_i = A_ini_i; A_index_i < A_limit_i; A_index_i += g_cacheBlockSize) {
        ...
        aux_limit_i = min((A_index_i+g_cacheBlockSize), A_limit_i);
 		...
 		if (A_index_i >= A_morton_next_block) {
			A_morton_i++;
	        C_morton_i++;	
	        A_morton_next_block += g_mortonBlockSide;
	        change_C = 1;
	        change_A = 1;
		}
        ...
        B_morton_next_block = B_aux_next_block;
        change_C = 1;
        change_B = 1;
    	...
    	for (B_index_j = B_ini_j; B_index_j < B_limit_j; B_index_j += g_cacheBlockSize) {
    		...
    		if (B_index_j >= B_morton_next_block) {
				B_morton_j++;
	        	C_morton_j++;
			    B_morton_next_block += g_mortonBlockSide;
			    change_C = 1;
			    change_B = 1;
			}
    	    ...
    	    int K_morton_next_block = g_mortonBlockSide;
    		...
    		for (k = 0; k < limit_k; k += g_cacheBlockSize) {
				...
				if (k >= K_morton_next_block) {
					A_morton_j++;
	       	 		B_morton_i++;
					K_morton_next_block += g_mortonBlockSide;
					change_A = 1;
					change_B = 1;
				}	
                ...
                int A_left = (A_index_i-A_ini_i)%g_mortonBlockSide;
                int B_left = (B_index_j-B_ini_j)%g_mortonBlockSide;
                int B_left_aux = B_left;
                int A_blocks = A_left * g_mortonBlockSide;
                int B_blocks = B_left * g_mortonBlockSide;
                
                int dif_i = A_index_i-A_ini_i;
              	for(aux_i = A_index_i; aux_i < aux_limit_i; ++aux_i, ++A_left, A_blocks += g_mortonBlockSide) {
              		
              		if (A_left == g_mortonBlockSide)
              			A_blocks = 0;
           			
           			B_left = B_left_aux;
           			B_blocks = B_left * g_mortonBlockSide;
           			
                	for(aux_j = B_index_j; aux_j < aux_limit_j; ++aux_j, ++B_left, B_blocks += g_mortonBlockSide) {
                    	acc0 = 0; acc1 = 0; acc2 = 0; acc3 = 0; acc4 = 0;
                    	
						if (B_left == g_mortonBlockSide)
              				B_blocks = 0;
						
						// Update real indexes inside morton block.
						int finalIndexA = A_realInd + A_blocks;
						int finalIndexB = B_realInd + B_blocks;
						
						int K_unroll_morton_next_block = K_morton_next_block;
						int real_k = k - (K_unroll_morton_next_block - g_mortonBlockSide);
						
						// Unrolling for k loop
                    	for(aux_k = k; aux_k < unroll_limit; aux_k+=unroll_factor, real_k+=unroll_factor) {
                    		// Index k inside the morton block
                    		if (aux_k >= K_unroll_morton_next_block) {
								real_k = aux_k - K_unroll_morton_next_block; 
								K_unroll_morton_next_block += g_mortonBlockSide;
							}

                        	acc0 += A (finalIndexA+real_k) * B (finalIndexB+real_k);
                        	acc1 += A (finalIndexA+real_k+1) * B (finalIndexB+real_k+1);
                        	acc2 += A (finalIndexA+real_k+2) * B (finalIndexB+real_k+2);
                        	acc3 += A (finalIndexA+real_k+3) * B (finalIndexB+real_k+3);
                        	acc4 += A (finalIndexA+real_k+4) * B (finalIndexB+real_k+4);
                        } 
                        
                        // Update real indexes inside morton block.
                        int finalIndexC = C_realInd + A_blocks + B_left;
                        		           
                        // Gather possible uncounted elements
                        for (; aux_k < aux_limit_k; ++aux_k, ++real_k){
                        	// Index k inside the morton block
                        	if (aux_k >= K_unroll_morton_next_block) {
								real_k = aux_k - K_unroll_morton_next_block; 
								K_unroll_morton_next_block += g_mortonBlockSide;
							}
                        	C(finalIndexC) += A (finalIndexA+real_k) * B (finalIndexB+real_k); 	
						}
