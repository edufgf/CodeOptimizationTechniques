                	...
                	for(aux_j = B_index_j; aux_j < aux_limit_j; ++aux_j) {
                    	...
                    	for(aux_k = k; aux_k < unroll_limit; aux_k+=unroll_factor) {
                        	...
                        	acc = _mm256_add_pd(acc, 
                        		  _mm256_mul_pd( _mm256_load_pd( A.getAddress(finalIndexA+mod_aux_k) ), 
                        		  			     _mm256_load_pd( B.getAddress(finalIndexB+mod_aux_k) ) ) );
                        } 
                        ...
                        C(finalIndexC) += acc_vet[0] + acc_vet[1] + acc_vet[2] + acc_vet[3];
...
void allocateMatrices(Array2D<E> &matrix_A, Array2D<E> &matrix_B, Array2D<E> &matrix_C, int m, int k, int n) {
	...
	matrix_A.build(A_side, A_side, 1, 0);
	matrix_B.build(B_side, B_side, 1, 0);
	matrix_C.build(C_side, C_side, 1, 0);
}
...
void writeOutput(Array2D<E> &matrix, int m, int n){
    ofstream file_out;
		...
		file_out.open("recursiveMult2_matrix_C.txt");
	#else
		file_out.open("recursiveMultParallel2_matrix_C.txt");	
	#endif
...
void writeReadTime(double time){
	ofstream file_out;
	#ifdef FORCE_SINGLE_CORE
		file_out.open("recursiveMult2_read_time.txt");
	#else
		file_out.open("recursiveMultParallel2_read_time.txt");
	#endif
	if (file_out.is_open()) {
		file_out << time;
		file_out.close();
...
void writeWriteTime(double time){
	ofstream file_out;
	#ifdef FORCE_SINGLE_CORE
		file_out.open("recursiveMult2_write_time.txt");
	#else
		file_out.open("recursiveMultParallel2_write_time.txt");
	#endif
	if (file_out.is_open()) {
		file_out << time;
		file_out.close();
...
void writeProcessTime(double time){
	ofstream file_out;
	#ifdef FORCE_SINGLE_CORE
		file_out.open("recursiveMult2_process_time.txt");
	#else
		file_out.open("recursiveMultParallel2_process_time.txt");
	#endif
	if (file_out.is_open()) {
		file_out << time;
		file_out.close();
...
int main() {	
	...
	Array2D<E> matrix_A(0, 0, 1);
	Array2D<E> matrix_B(0, 0, 1);
	Array2D<E> matrix_C(0, 0, 1);
