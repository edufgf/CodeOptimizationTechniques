...
void writeOutput(Array2D<E> &matrix_C, int dim1, int dim2) {
	ofstream file_out;
	file_out.open("standardOpenCL_matrix_C.txt");
...
void writeReadTime(double time){
	ofstream file_out;
	file_out.open("standardOpenCL_read_time.txt");
...
void writeWriteTime(double time){
	ofstream file_out;
	file_out.open("standardOpenCL_write_time.txt");
...
void writeProcessTime(double time){
	ofstream file_out;
	file_out.open("standardOpenCL_process_time.txt");
...
void writeProcessTimeReal(double time){
	ofstream file_out;
	file_out.open("standardOpenCL_process_time_real.txt");
...
int main() {
	...
	// Read kernel file
	string kernel_str;
	ifstream file_input("openCL/standardOpenCL_double.cl");
	stringstream buffer;
    ...
    // Clean Buffer 
    // double zero = 0.0;
	// clEnqueueFillBuffer ( commands , d_c, &zero, sizeof(E), 0, sizeof(E) * m*n, 0 , NULL,NULL);
