...
public:
		            ...
		            unroll_limit = aux_limit_k - (unroll_factor-1); // Unrolling by factor of 5
		          	...
		          	for(aux_i = i; aux_i < aux_limit_i; ++aux_i) {
		            	for(aux_j = j; aux_j < aux_limit_j; ++aux_j) {
		                	...
		                	E acc0 = 0; E acc1 = 0; E acc2 = 0; E acc3 = 0; E acc4 = 0;
	...
	void writeOutput(E** matrix_C, int dim1, int dim2) {
		ofstream file_out;
		file_out.open("optimizedMultParallel_matrix_C.txt");
		if (file_out.is_open()) {
			file_out << std::fixed;
			for (int i = 0; i < dim1; ++i) {
				for (int j = 0; j < dim2; ++j) {
	...
	void writeReadTime(double time){
		ofstream file_out;
		file_out.open("optimizedMultParallel_read_time.txt");
		if (file_out.is_open()) {
			file_out << time;
			file_out.close();
		}	
	...
	void writeWriteTime(double time){
		ofstream file_out;
		file_out.open("optimizedMultParallel_write_time.txt");
		if (file_out.is_open()) {
			file_out << time;
			file_out.close();
		}	
	...
	void writeProcessTime(double time){
		ofstream file_out;
		file_out.open("optimizedMultParallel_process_time.txt");
		if (file_out.is_open()) {
			file_out << time;
			file_out.close();
		}	
