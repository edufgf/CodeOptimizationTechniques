...
public:
	...
	void writeOutput(E** matrix_C, int dim1, int dim2) {
		ofstream file_out;
		file_out.open("standardMultOptimized_matrix_C.txt");
		if (file_out.is_open()) {
			file_out << std::fixed;
			for (int i = 0; i < dim1; ++i) {
				for (int j = 0; j < dim2; ++j) {
	...
	void writeReadTime(double time){
		ofstream file_out;
		file_out.open("standardMultOptimized_read_time.txt");
		if (file_out.is_open()) {
			file_out << time;
			file_out.close();
		}	
	...
	void writeWriteTime(double time){
		ofstream file_out;
		file_out.open("standardMultOptimized_write_time.txt");
		if (file_out.is_open()) {
			file_out << time;
			file_out.close();
		}	
	...
	void writeProcessTime(double time){
		ofstream file_out;
		file_out.open("standardMultOptimized_process_time.txt");
		if (file_out.is_open()) {
			file_out << time;
			file_out.close();
		}	
